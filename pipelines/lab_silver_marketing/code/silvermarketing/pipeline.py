...
def pipeline(spark: SparkSession) -> None:
    ...
    df_campaign_revenue_summary = campaign_revenue_summary(spark, df_JoinSalesDate)
    df_SumCosts = SumCosts(spark, df_bronze_marketing_events)
    df_outer_join_campaigns = outer_join_campaigns(
        spark, 
        df_SumCosts, 
        df_campaign_revenue_summary, 
        df_bronze_marketing_campaigns
    )
    target_dim_campaig(spark, df_outer_join_campaigns)
